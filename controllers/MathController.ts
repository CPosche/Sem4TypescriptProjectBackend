...
const sortItemsByPriority = (items: TItem[], calcData: calcData): TItem[] => {
  return items.sort((a, b) => {
    for (const stat of calcData.calcData.statPriority) {
      const aValue =
        a.preview_item.stats.find((s: TStat) => s.type.type === stat)?.value ||
        0;
      const bValue =
        b.preview_item.stats.find((s: TStat) => s.type.type === stat)?.value ||
        0;
...
const getItemsFromStatWeight = async (calcData: calcData) => {
  ...
  for (const [key, value] of dungeonMap) {
    const filteredItems = value.filter((item) => {
      return (
        item.preview_item.item_subclass.name ===
        calcData.calcData.classData.armorType
      );
    });
    dungeonMap.set(key, sortItemsByPriority(filteredItems, calcData));
  }
